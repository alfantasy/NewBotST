...
from lib.config import token_for_bot as tkn
...
async def remove_item(message: types.Message):
    user_id = message.from_user.id 
    args = message.get_args()
    result = player_inventory.sell_item(player_id=user_id, item_id=args)
    await message.answer(result)
...
async def add_item(message: types.Message):
    user_id = message.from_user.id 
    args = message.get_args()
    result = player_inventory.buy_item(player_id=user_id, item_id=args)
    await message.answer(result)
